...
The content consumer, our secondary persona, who would like to learn or practice, can watch this "movie", can stop and restart. They can follow the editing process and can see the comments. At the end of the tutorial, the content consumer can click in the code editor and can edit.
...
Our secondary persona, the content consumer can play and stop the tutorial, can choose individual step and can use the code editor for experimenting with the example. From their perspective, the CodeMirror Movie is almost an ideal tool.
...
A good code editor is an important part of a tutorial builder. The content creator persona prefers a code editor which helps editing, can autocomplete, can save the history of the editing process and supports undo and redo features.
...
We have seen in our previous chapter, that there are tools, which provide partially solution for tutorial builders and tutorial consumers. However a combination of these tools would support more our primary and secondary personas.
...
More about the implementation in Section \ref{breadcrumbs-implementation}.
...
\ref{fig:database-tables} summarizes these database tables and models.
...
Expert evaluators are my colleagues. 4 developers and 1 UX designer. In each case I followed the same process. I showed them my prototype and I asked them to build a simple tutorial. The task was to create a tutorial with 3-4 steps which shows how can you build a basic Hello World website with Bootstrap. I made notes about their feedback, questions and difficulties.
...
Here is a walkthrough on how the app works, what it does and feedbacks from the evaluators.
...
Originally, the app did not have any home page. Based on the feedbacks from the expert evaluators, there is a home screen with the title. It is mainly a placeholder. Originally, there was not any homepage, but all evaluators mentioned the lack of it.
...
Reviewing this area with evaluators, one of the most important feedbacks was, how we can modify, drag and drop, delete steps; and how we can keep the consistency between steps. These feedbacks reflect the same questions that we described in the Chapter \ref{history}. 4 from 5 evaluators wanted to reorganize steps with drag and drop. 3 evaluators deleted steps and recognized the inconsistency between steps after this action. They manually adjusted the content in the connected steps, to keep the consistency again.
...
The survey focuses on our secondary persona. I tried to reach participants who more likely are tutorial content consumers, students, developers. For this reason, I shared the link on my tutorial website (\url{www.yoember.com}). This website's visitors are in our target group. I shared the link on Slack groups, what are mainly used by students or developers who would like to learn new skills. (Enspiral Dev Academy Slack Channel, Yoember Slack Channel, JavaScript New Zealand Slack Channel). Twitter was also efficient in the participant recruitment process as we can see in the Figure \ref{fig:twitter}
